...
import author1 from "../../assets/avt/author1.webp";
      ...
      >
        <div>
               {testimonials.map((item, idx) => (
                <SwiperSlide key={idx}>
                  <div className="container-card bg-white p-6 rounded-2xl hover:shadow-xl transition-shadow h-full flex flex-col items-center text-center" style={{width:"523px",height:"312px"}} >
  
                        <div className="flex align-center" style={{marginRight:'200px'}}>
                        <img style={{width:'65px', height:"65px"}}
                        src={item.avatar}
                        alt={item.name}
                        className="w-20 h-20 rounded-full object-cover mb-4"
                        />
                        <h4 className="font-bold text-lg">{item.name}</h4></div>
                        <p style={{width:'460.33px'}} className="text-start">{item.text}</p>
